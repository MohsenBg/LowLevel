...
#define IOCTL_TEST_START CTL_CODE_HIDE(1)  // IOCTL code for starting the test
    ...
    NTSTATUS WDM_STUPID_DeviceCreateRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp);
    NTSTATUS WDM_STUPID_DeviceCloseRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp);
    NTSTATUS WDM_STUPID_DeviceShutdownRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp);
    NTSTATUS WDM_STUPID_DeviceControlRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp);
    ...
    NTSTATUS DriverEntry(
        ...
        if (DriverObject->DriverExtension != NULL) {
            // Set up driver unload and add device functions
            DriverObject->DriverUnload = WDM_STUPID_DriverUnload;
            DriverObject->DriverExtension->AddDevice = DriverAddDevice;
        }
        else {
            // Print error if DriverExtension is NULL
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "Driver Extension is NULL\n"));
    ...
    NTSTATUS DriverAddDevice(
        ...
        if (!NT_SUCCESS(status)) {
            ...
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "IO CreateDevice failed(0x%x)", status));
            return status;
        }
        ...
        if (!NT_SUCCESS(status)) {
            ...
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "IO CreateSymbolicLink failed(0x%x)", status));
            return status;
        }
    ...
    NTSTATUS WDM_STUPID_DeviceCreateRoutine(_In_ PDEVICE_OBJECT DeviceObject, _Inout_ PIRP Irp) {
        ...
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "IRP_MJ_CREATE\n"));
    ...
    NTSTATUS WDM_STUPID_DeviceCloseRoutine(_In_ PDEVICE_OBJECT DeviceObject, _Inout_ PIRP Irp) {
        UNREFERENCED_PARAMETER(DeviceObject);
        UNREFERENCED_PARAMETER(Irp);
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "IRP_MJ_CLOSE\n"));
        return STATUS_SUCCESS;
    }
    ...
    NTSTATUS WDM_STUPID_DeviceShutdownRoutine(_In_ PDEVICE_OBJECT DeviceObject, _Inout_ PIRP Irp) {
        UNREFERENCED_PARAMETER(DeviceObject);
        UNREFERENCED_PARAMETER(Irp);
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "IRP_MJ_SHUTDOWN\n"));
        return STATUS_SUCCESS;
    }
    ...
    NTSTATUS WDM_STUPID_DeviceControlRoutine(_In_ PDEVICE_OBJECT DeviceObject, _Inout_ PIRP Irp) {
        ...
        case IOCTL_TEST_START:
            ...
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "IOCTL called= %x \r\n", *(PLONG)inputBuffer));
